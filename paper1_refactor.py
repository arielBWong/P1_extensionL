...
def pfnd2csv(pf_nd, target_problem, seed_index, method_selection, search_ideal, nadir_record, cornerid, prediction_xrecord, prediction_yrecord, extreme_search, success_extremesearch):
    ...
    '''
    if search_ideal == 3:
        savename = savefolder + os.sep + 'idealsearchid_seed_' + str(seed_index) + '.csv'
        ...
        savename = savefolder + os.sep + 'prediction_xrecord_seed_' + str(seed_index) + '.csv'
        np.savetxt(savename, prediction_xrecord, delimiter=',')

        prediction_yrecord = prediction_yrecord.reshape(-1, target_problem.n_obj)
        savename = savefolder + os.sep + 'prediction_yrecord_seed_' + str(seed_index) + '.csv'
        np.savetxt(savename, prediction_yrecord, delimiter=',')

        savename = savefolder + os.sep + 'search_and_success_rate' + str(seed_index) + '.csv'
...
def usage(prog):
    print('Usage: %s single|para' % prog)
    sys.exit(1)
